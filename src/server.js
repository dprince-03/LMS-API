...
// Generated by `npm run docs:generate` (see scripts/generate-docs.js) — not
// present until that's run at least once, so missing it shouldn't crash the
// server, just disable the interactive docs endpoint.
const openapiSpec = (() => {
	try {
		return require("./openapi.json");
	} catch (_error) {
		logger.warn(
			"src/openapi.json not found — run `npm run docs:generate` to enable /api/docs. Continuing without it."
		);
		return null;
	}
})();
...
app.get("/api", limiter, (req, res) => {
	res.status(200).json({
		...
		version: "1.0.0",
		documentation: openapiSpec ? "/api/docs" : "/docs/API.md",
		health: "/api/health",
	});
});
...
if (openapiSpec) {
	app.use(
		"/api/docs",
		swaggerUi.serve,
		swaggerUi.setup(openapiSpec, { customSiteTitle: "LMS API Docs" })
	);
