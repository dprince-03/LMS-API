...
describe("Full Library Workflow", () => {
	...
	it("should complete full borrow-return cycle", async () => {
		...
		expect(borrowResponse.body.data.borrow_record).toHaveProperty("id");

		// 2. Check book availability decreased
		const bookResponse = await request(app).get(`/api/books/${book.id}`).expect(200);
		...
		const finalBookResponse = await request(app).get(`/api/books/${book.id}`).expect(200);
